...

{
	...
	// 将初始化好的学生插入尾部
	//list->stu[list->last+1]=stu1; // 写法一  正确
	strcpy(list->stu[list->last+1].name,stu1.name); // 写法二    正确
	list->stu[list->last+1].score=stu1.score; //
	// 更新last的值
	list->last++;
	return success;
	
} */
struct slist slist_tailinsert(struct slist list) //???
...
int main()
{
	...
	// 往初始化好的顺序表中插入新的学生
	for(i=0; i<2; i++)
		*listp=slist_tailinsert(*listp); // 非要使用值传递的代价
